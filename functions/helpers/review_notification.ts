...
export async function codeReviewNotification(
  ...
  if (event_payload.author === responsibleUser) return
    ...
    blocks: [
      {
        ...
        text: {
          type: 'mrkdwn',
          text: `${icon} <@${event_payload.author}>, ` + notificationMessage
        }
      }
    ],
...
export async function codeReviewNotificationBackup(
  client: SlackAPIClient,
  channelId: string,
  timestamp: string,
  responsibleUser: string,
...
function getNotificationMessage(action_id: string, event_payload: EventPayload, currentUserId: string) {
  ...
  if (action_id === 'unapprove') {
    return `<@${currentUserId}> Re-Opened (Unapproved) the <${event_payload.pr_url}|Pull Request> for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>.`
  }

  if (action_id === 'approve') {
    return `<@${currentUserId}> Approved the <${event_payload.pr_url}|Pull Request> for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>.`
  }

  if (action_id === 'undecline') {
    return `<@${currentUserId}> Re-Opened (Un-Declined) the <${event_payload.pr_url}|Pull Request> for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>.`
  }

  if (action_id === 'decline') {
    return `<@${currentUserId}> Declined the <${event_payload.pr_url}|Pull Request> for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>`
  }

  if (action_id === 'claim') {
    return `<@${currentUserId}> Claimed the <${event_payload.pr_url}|Pull Request> for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>`
  }

  if (action_id === 'unclaim') {
    return `<@${currentUserId}> Unclaimed the <${event_payload.pr_url}|Pull Request> for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>`
  }

  if (action_id === 'mark') {
    return `<@${currentUserId}> Marked the <${event_payload.pr_url}|Pull Request> as '${event_payload.mark}' for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>`
  }

  if (action_id === 'unmark') {
    return `<@${currentUserId}> Unmarked the <${event_payload.pr_url}|Pull Request> for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>`
  }

  if (action_id === 'edit') {
    return `<@${currentUserId}> Edited the <${event_payload.pr_url}|Pull Request> Review for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>`
  }

  if (action_id === 'subscribe') {
    return `You've Subscribed to the <${event_payload.pr_url}|Pull Request> Review for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>, Welcome to the party!`
  }

  if (action_id === 'unsubscribe') {
    return `You've Unsubscribed to the <${event_payload.pr_url}|Pull Request> Review for <${issueUrlPrefix}${event_payload.issue_id}|${event_payload.issue_id}>, Goodbye!`
  }
}
